test_that("rbin constant beta_intercepts", {
  ...
  simulated_binary_dataset <- rbin(
    clsize = cluster_size, intercepts = beta_intercepts,
...
test_that("rmult.bcl constant beta_coefficients", {
  ...
  simulated_nominal_dataset <- rmult.bcl(
    clsize = cluster_size, ncategories = categories_no,
  ...
  lin_pred <- matrix(beta_coefficients, nrow(xmat), ncol(xmat), TRUE) * xmat
...
test_that("rmult.bcl varying beta_coefficients", {
  ...
  beta_coefficients <- matrix(
    c(
      1, 3, 2, 1.25, 3.25, 1.75, 0.75, 2.75, 2.25, 0, 0, 0,
  ...
  xmat <- model.matrix(~ x1 + x2, data = xdata)
  xmat <- apply(xmat, 2, function(x) rep(x, each = categories_no))
  lin_pred <- matrix(c(t(beta_coefficients)), nrow(xmat), ncol(xmat), TRUE) *
    xmat
  lin_pred <- rowSums(lin_pred) + c(t(simulated_nominal_dataset$rlatent))
  lin_pred <- matrix(lin_pred, sample_size * cluster_size, categories_no, TRUE)
  simulated_responses <- apply(lin_pred, 1, which.max)
...
test_that("rmult.acl constant beta_coefficients", {
  ...
  set.seed(1)
  latent_correlation_matrix <- kronecker(
    toeplitz(c(1, rep(0.95, cluster_size - 1))), diag(4)
  )
  ...
  beta_coefficients_bcl <- c(
    beta_intercepts[1], 3, 3, beta_intercepts[2], 2, 2,
    beta_intercepts[3], 1, 1, beta_intercepts[4], 0, 0
  )
  set.seed(1)
  simulated_nominal_dataset <- rmult.bcl(
    clsize = cluster_size, ncategories = 4, betas = beta_coefficients_bcl,
    xformula = ~ x1 + x2, xdata = xdata, cor.matrix = latent_correlation_matrix
  )
  expect_equal(
    c(t(simulated_ordinal_dataset$Ysim)), c(t(simulated_nominal_dataset$Ysim))
  )
})
...
test_that("rmult.crm constant beta_coefficients", {
  ...
  latent_correlation_matrix <- diag(1, (categories_no - 1) * cluster_size) +
    kronecker(
      toeplitz(c(0, rep(0.24, categories_no - 2))),
  ...
  simulated_latent_responses <- c(t(simulated_ordinal_dataset$rlatent)) -
    rep(x, each = categories_no - 1)
  simulated_responses <- matrix(
    as.numeric(t(simulated_latent_responses <= beta_intercepts)),
