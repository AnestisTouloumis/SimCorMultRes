rbin <- function(clsize = clsize, intercepts = intercepts, betas = betas, xformula = formula(xdata), xdata = parent.frame(), link = "logit", cor.matrix = cor.matrix, 
    rlatent = NULL) {
    ...
    for (i in 1:clsize) Ysim[, i] <- cut(U[, i] - 2 * lin.pred[, i], intercepts[i, ], labels = FALSE)
    Ysim <- 2 - Ysim
