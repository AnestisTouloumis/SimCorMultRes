...
## No covariates in the marginal model
Simulation of clustered binary, ordinal and nominal responses in the case where the marginal probabilities are not defined by a marginal model can be achieved by performing the following four steps:

1. Create a pseudo-covariate say `x` of length equal to the number of cluster size (`clsize`) times the desired number of clusters of simulated responses (say `R`), that is `x = clsize * R`. Definition of a zero valued covariate `x` is required to identify the desired number of clustered responses.  
...
See also the third example in @Touloumis2016 in order to connect a marginal model with the desired marginal probabilities.

```{example, label=no-covariate, name="Simulation of clustered binary responses without covariates"}
Suppose the goal is to simulate $N=5000$ clustered binary responses with $\Pr(Y_{it}=1)=0.8$ for all $t=1,\ldots,4$. For the association structure, assume that the random variables $\mathbf{e}_i^{B}=(e^{B}_{i1},e^{B}_{i2},e^{B}_{i3},e^{B}_{i4})^{\prime}$ in \@ref(eq:TMB) are iid random vectors from the tetra-variate normal distribution with mean vector the zero vector and covariance matrix the correlation matrix $\mathbf{R}$ is given in Example \@ref(ex:BinaryProbit).
```
