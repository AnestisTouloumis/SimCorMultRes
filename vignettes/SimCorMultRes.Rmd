---
title: "Simulating Correlated Binary and Multinomial Responses with __SimCorMultRes__"
...
# Introduction 
The __R__ package __SimCorMultRes__ is suitable for simulation of correlated binary responses (exactly two response categories) and of correlated nominal or ordinal multinomial responses (three or more response categories) conditional on a regression model specification for the marginal probabilities of the response categories. This vignette briefly describes the simulation methods proposed by @Touloumis2016 and illustrates the use of the core functions of __SimCorMultRes__. A more detailed description of __SimCorMultRes__ can be found in @Touloumis2016. 

# Motivation
The package __SimCorMultRes__ facilitates the task of carrying out simulation studies to evaluate the performance of statistical method(s) for estimating the regression parameters in a marginal model with clustered (correlated) binary and multinomial responses. Examples of such statistical methods include maximum likelihood methods, copula approaches, quasi-least squares approaches, generalized quasi-likelihood methods and generalized estimating equations (GEE) approaches among others [see references in @Touloumis2016]. However, it is worth mentioning that __SimCorMultRes__ can also be used to generate correlated binary and multinomial random variables conditional on a desired dependence structure and known marginal probabilities that are not determined by a regression model [see the third example in @Touloumis2016].
...
For each subject $i$, the association structure among the clustered nominal responses $\{Y_{it}:t=1,\ldots,T\}$ depends on the joint distribution and correlation matrix of $\{e^{NO}_{itj}:t=1,\ldots,T \text{ and } j=1,\ldots,J\}$. If the random variables $\{e^{N}_{itj}:t=1,\ldots,T \text{ and } j=1,\ldots,J\}$ are independent then so are $\{Y_{it}:t=1,\ldots,T\}$.
\newline{}

```{example, Example1, name="Simulation of clustered nominal responses conditional on a marginal baseline-category logit model using the NORTA method"}
...
0.95 & \text{if } t_1=t_2 \text{ and } j_1=j_2\\
0 & \text{otherwise }\\
\end{cases}
\]
for all $i=1,\ldots,N$.
...
clsize <- 3 
set.seed(1) 
# time-stationary covariate x1
x1 <- rep(rnorm(N), each = clsize)
# time-varying covariate xit2
x2 <- rnorm(N * clsize) 
...
\end{equation*}
with $N=500$, $T=4$, $(\beta_{10},\beta_{20},\beta_{30},\beta_{40},\beta)=(-1.5,-0.5,0.5,1.5,1)$ and $\mathbf{x}_{it}=x_{it}\overset{iid}{\sim} N(0,1)$ for all $i$ and $t$ is desired. For the dependence structure, assume that $\{\mathbf{e}_i^{O2}=\left(e^{O2}_{i11},\ldots,e^{O1}_{i44}\right)^{\prime}:i=1,\ldots,N\}$ are iid random vectors from a multivariate normal distribution with mean vector the zero vector and covariance matrix the $16 \times 16$ correlation matrix with elements\[
...
## No covariates in the linear predictor of the marginal model
Simulation of clustered binary, ordinal and nominal responses in the absence of covariates in the linear predictor of the desired marginal model can be achieved by performing the following four steps:

1. Create a pseudo-covariate say `x` of length equal to the number of cluster size (`clsize`) times the desired number of clusters of simulated responses (say `R`), that is `x = clsize * R`. Definition of a zero valued covariate `x` is required in order to define the desired number of clustered responses.  
1. Set `betas = 0` in the core functions `rbin`, `rmult.clm` and `rmult.crm` (see Example \@ref(ex:no-covariate)), or set 0 all values of the `beta` argument that correspond to the category-specific parameters in the core function `rmult.bcl` (see Example \@ref(ex:Example2NoCovariates)).
1. set `xformula = ~ x`.
1. Run the core function to obtain realizations of the simulated clustered responses.

```{example, label=no-covariate, name="Simulation of clustered binary responses without covariates"}
Suppose the goal is to simulate clustered binary responses from the marginal probit model 
$$\Pr(Y_{it}=1 |\mathbf{x}_{it})=\Phi(0.2)$$
where $N=5000$, $T=4$. For the association structure, assume that the random variables $\mathbf{e}_i^{B}=(e^{B}_{i1},e^{B}_{i2},e^{B}_{i3},e^{B}_{i4})^{\prime}$ in \@ref(eq:TMB) are iid random vectors from the tetra-variate normal distribution with mean vector the zero vector and covariance matrix the correlation matrix $\mathbf{R}$ is given in Example \@ref(ex:BinaryProbit).
```
...
library(gee)
# fitting a gee model
binGEEmod <- gee(y ~ 1, family = binomial("probit"), id = id, data = CorBinRes$simdata)
# checking the estimated coefficients
summary(binGEEmod)$coefficients
```
...
Suppose the aim is to simulate nominal responses from the marginal baseline-category logit model
\begin{equation*}
\log \left[\frac{\Pr(Y_{it}=j |\mathbf {x}_{it})}{\Pr(Y_{it}=4 |\mathbf {x}_{it})}\right]=\beta_{j0}
\end{equation*}
where $N=500$, $T=3$, $(\beta_{10},\beta_{20},\beta_{30})=(1, 1.25, 0.75)$. For the dependence structure, suppose that the correlation matrix $\mathbf{R}$ in the NORTA method is the same as in Example \@ref(ex:Example1). 
```
...
suppressPackageStartupMessages(library("multgee"))
# fitting a gee model
fit <- nomLORgee(y~1,data=CorNorRes$simdata,id=id,repeated=time,LORstr="time.exch")
# checking regression coefficients
round(coef(fit),2)
```
# How to Cite

A considerable effort has been put into developing this package. Please use the function `citation` in order to cite **SimCorMultRes** in publications: 
```{r}
