...
## Warning: No covariates in the linear predictor
If there are no covariates in the linear predictor of the desired marginal model, then follow the following three steps:

1. Create a pseudo-covariate say `x` of length equal to the number of cluster size (`clsize`) times the number of desired clusters of simulated responses (say `R`), that is `x = clsize * R`,
1. set `betas = 0` in the core functions `rbin`, `rmult.clm` and `rmult.crm`, or set 0 all values of the `beta` argument that correspond to the category-specific parameters in the core function `rmult.bcl`.
1. set `xformula = ~ x`
...
where $N=5000$, $T=4$. For the association structure, assume that the random variables $\mathbf{e}_i^{B}=(e^{B}_{i1},e^{B}_{i2},e^{B}_{i3},e^{B}_{i4})^{\prime}$ in \@ref(eq:TMB) are iid random vectors from the tetra-variate normal distribution with mean vector the zero vector and covariance matrix the correlation matrix $\mathbf{R}$ given by
\begin{equation}
\mathbf{R}=\left( {\begin{array}{*{20}c}
   1.00 & 0.90 & 0.90 & 0.90  \\
   0.90 & 1.00 & 0.90 & 0.90  \\
...
\end{equation*}
where $N=500$, $T=3$, $(\beta_{10},\beta_{20},\beta_{30})=(1, 1.25, 0.75)$. For the dependence structure, suppose that the correlation matrix $\mathbf{R}$ in the NORTA method has elements 
\[
\mathbf{R}_{t_1j_1,t_2j_2}=\begin{cases}
1 & \text{if } t_1=t_2 \text{ and } j_1=j_2\\
0.95 & \text{if } t_1=t_2 \text{ and } j_1=j_2\\
